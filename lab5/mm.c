...
/*function parallels:
extend_heap = requestMoreSpace
find_fit = searchFreeList
coalescse = coalesceFreeBlock
place = insertFreeBlock
remove_freeblock = removeFreeBlock
*/
...
void *mm_malloc(size_t size)
{
    ...
    //If the current free space is not enough for the block, request it
    ...
    //save tags and remove old space from free since it is now allocated
    precedingBlockUseTag = ptrFreeBlock->sizeAndTags & TAG_PRECEDING_USED;

    blockSize = SIZE(ptrFreeBlock->sizeAndTags);
    removeFreeBlock(ptrFreeBlock);

    //If we need more than the curr block space allows, then split the block
    if (blockSize - reqSize >= MIN_BLOCK_SIZE)
    {
        ptrFreeBlock->sizeAndTags = reqSize | precedingBlockUseTag;
        ptrFreeBlock->sizeAndTags = ptrFreeBlock->sizeAndTags | TAG_USED;

        *((size_t *)UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize)) = (blockSize - reqSize) | TAG_PRECEDING_USED;
        *((size_t *)UNSCALED_POINTER_ADD(ptrFreeBlock, blockSize - WORD_SIZE)) = (blockSize - reqSize) | TAG_PRECEDING_USED;

        // Add freed block into free list
        insertFreeBlock((BlockInfo *)UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize));
    }
    else
    {
        //Set next block's tags to used
...
void mm_free(void *ptr)
{
    ...
    //Next block preceding tag becomes 0
    followingBlock->sizeAndTags = followingBlock->sizeAndTags & (~TAG_PRECEDING_USED);

    //Used tag becomes 0 and updates boundary tag
    ...
    *((size_t *)UNSCALED_POINTER_ADD(blockInfo, payloadSize - WORD_SIZE)) = blockInfo->sizeAndTags;
    ...
    //Block is now freed and coalesced with surrounding blocks
